        ...
        <tbody>
            @foreach ($data as $item)
                <tr>
                    ...
                    <td>{{ $item['nome'] }}</td>
                    <td>{{ $item['carga'] . ' Horas' }}</td>

                    @foreach($cursos as $dados)
                        @if( $item['curso_id'] == $dados->id)
                            <td class="d-none d-md-table-cell">{{ $dados['nome'] }}</td>
                        @endif
                    @endforeach
                    <td>
                        <a href= "{{ route('disciplinas.edit', $item) }}" class="btn btn-success">
                        ...
                        <a href= "{{route('disciplina_professors.index')}}" class="btn btn-warning">
                    ...
                    <form action="{{ route('disciplinas.destroy', $item) }}" method="POST" id="form_{{$item['id']}}">
                        @csrf
                        @method('DELETE')
                    </form>
                </tr>
